...
describe('first-batch UI regression contracts', () => {
  ...
  it('keeps split-pane empty actions vertically centered without dashed chrome', () => {
    ...
    expect(terminalEmptyPaneSource).not.toContain('action-separator')
  ...
  it('keeps beta smoke UI cleanup contracts explicit', () => {
    ...
    expect(workspaceTabsSource).not.toContain('class="topbar-split"')
    expect(workspaceTabsSource).not.toContain('topbar-action-separator')
    expect(workspaceTabsSource).toContain('data-split-menu-toggle')
    ...
    expect(css).not.toContain('.topbar-action-separator')
