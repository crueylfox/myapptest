...
describe('first-batch UI regression contracts', () => {
  ...
  it('keeps split-pane empty actions vertically centered without dashed chrome', () => {
    ...
    expect(terminalEmptyPaneSource).toContain('action-separator')
    expect(terminalEmptyPaneSource).toContain('separatorClass')
  ...
  it('keeps beta smoke UI cleanup contracts explicit', () => {
    ...
    expect(workspaceTabsSource).toContain('class="topbar-split"')
    expect(workspaceTabsSource).toContain('topbar-action-separator')
    expect(workspaceTabsSource).not.toContain('data-split-menu-toggle')
    expect(block('.topbar-split > button')).toContain('border: 0')
    expect(block('.topbar-action-separator')).toContain('width: 1px')
    ...
    expect(block('.topbar-action-inner')).toContain('padding: 3px 7px')
    expect(block('.topbar-split > button:hover .topbar-action-inner, .topbar-split > button[aria-expanded="true"] .topbar-action-inner')).toContain('background: var(--panel-2)')
    expect(block('.topbar-navigation > button:hover .topbar-action-inner, .topbar-navigation > button[aria-expanded="true"] .topbar-action-inner')).toContain('background: var(--panel-2)')
    expect(block('.topbar-menu')).toContain('width: 180px')
    expect(block('.topbar-menu')).toContain('min-width: 180px')
    expect(block('.topbar-menu')).toContain('max-width: 180px')
    ...
    expect(sftpPanelSource).toContain("'sftp-empty-state': expanded && !online")
    expect(sftpPanelSource).not.toContain("'empty-state': expanded && !online")
    expect(block('.sftp-panel.sftp-empty-state')).toContain('grid-template-rows: minmax(0, 1fr)')
    expect(block('.sftp-panel.sftp-empty-state')).toContain('border-radius: 0')
